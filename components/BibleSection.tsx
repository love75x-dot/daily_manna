  ...
  return (
    <section className="bg-white rounded-2xl shadow-lg border border-stone-200 overflow-hidden mb-6">
      <div className="p-6 bg-stone-50 border-b border-stone-100">
        <label className="block text-stone-600 font-bold mb-2">📖 오늘의 말씀 찾기</label>
        <div className="flex gap-2">
          ...
          <button 
            onClick={onSearch}
            disabled={isLoading}
            className="px-6 py-3 bg-stone-800 hover:bg-stone-900 text-amber-50 rounded-lg font-bold transition-colors disabled:opacity-50"
          >
            {isLoading ? '검색 중...' : '검색'}
          </button>
        </div>
