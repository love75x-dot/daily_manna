...
대한성서공회 개역한글판 성경의 어투와 신학적 정통성을 유지하세요.
`;
...
export class GeminiService {
  ...
  async getBibleText(reference: string): Promise<string> {
    ...
    const prompt = `
    성경 본문 "${reference}"의 내용을 '개역한글' 판본 스타일로 출력해주세요.
    절 번호를 포함하여 텍스트만 깔끔하게 출력해주세요.
    다른 서론이나 본론 없이 오직 성경 텍스트만 출력하세요.
    `;
  ...
  async getMeditation(type: TabType, bibleText: string): Promise<string> {
    ...
    switch (type) {
      case TabType.OBSERVATION:
        prompt = `
        다음 성경 본문을 바탕으로 [관찰 질문 및 해설] 3가지를 작성해주세요.
        본문: ${bibleText}
      ...
      case TabType.INTERPRETATION:
        prompt = `
        다음 성경 본문을 바탕으로 [깊은 해석 질문 및 해설] 3가지를 작성해주세요.
        본문: ${bibleText}
        
        요구사항:
        1. 당시 시대적 배경이나 원어의 의미를 포함한 깊이 있는 해석.
      ...
      case TabType.APPLICATION:
        prompt = `
        다음 성경 본문을 바탕으로 구역 모임에서 나눌 수 있는 [실천 적용 질문] 3가지를 작성해주세요.
        본문: ${bibleText}
